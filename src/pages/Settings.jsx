...
export default function Settings() {
      ...
      try {
        ...
        if (resolvedCompanyId) {
          const companies = await base44.entities.Company.filter(
            { id: resolvedCompanyId, is_deleted: false },
            '-created_date',
            1
          ).catch(() => []);
  ...
  const saveCompanyProfile = async () => {
    if (!isAdmin) { showError('Only admins can update the company profile.'); return; }
  ...
  const saveLocalSettings = () => {
    if (!companyId) { showError('Company ID is required before saving settings.'); return; }
  ...
  const updateCompany = (key) => (event) => setCompanyForm((c) => ({ ...c, [key]: event.target.value }));
  const updateUser = (key) => (event) => setUserForm((c) => ({ ...c, [key]: event.target.value }));
  const updateLocal = (key, value) => setLocalSettings((c) => ({ ...c, [key]: value }));
        ...
        <>
              ...
              {activeTab === 'company' && (
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Used across jobs, documents, exports, and customer-facing records.</p>
                  </div>
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      <Save size={15} /> {saving === 'company' ? 'Saving...' : 'Save Company Profile'}
                    </button>
                  </div>
                  ...
                  <div className="border-t border-border pt-6 space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Your contact details and job title.</p>
                    </div>
                    ...
                    <div className="flex justify-end">
                      ...
                      >
                        <Save size={15} /> {saving === 'user' ? 'Saving...' : 'Save User Profile'}
                      </button>
                    </div>
                  </div>
                </div>
              ...
              {activeTab === 'notifications' && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Choose which alerts are enabled for this browser.</p>
                  </div>
                  <ToggleRow title="Email notifications" description="Enable general email notification preferences." checked={localSettings.email_notifications} onChange={(v) => updateLocal('email_notifications', v)} />
                  <ToggleRow title="Job activity notifications" description="Notify for new jobs, job updates, assigned work, and status changes." checked={localSettings.job_notifications} onChange={(v) => updateLocal('job_notifications', v)} />
                  <ToggleRow title="Approval notifications" description="Notify when estimates, supplements, and documents need approval." checked={localSettings.approval_notifications} onChange={(v) => updateLocal('approval_notifications', v)} />
                  <ToggleRow title="Billing notifications" description="Notify admins for plan, payment, and billing issues." checked={localSettings.billing_notifications} onChange={(v) => updateLocal('billing_notifications', v)} />
                  <div className="flex justify-end">
                    <button type="button" onClick={saveLocalSettings} className="inline-flex items-center gap-2 min-h-touch px-4 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition">
              ...
              {activeTab === 'security' && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Control company-level safety settings.</p>
                  </div>
                  {!isAdmin && (
                    <div className="rounded-lg border border-yellow-300 bg-yellow-50 p-3 text-sm text-yellow-800">
                  ...
                  <ToggleRow title="Require secure exports" description="Users must be signed in before opening job packets, estimates, and document exports." checked={localSettings.require_secure_exports} onChange={(v) => updateLocal('require_secure_exports', v)} />
                  <ToggleRow title="Session warning" description="Warn users before they leave forms with unsaved work." checked={localSettings.session_warning_enabled} onChange={(v) => updateLocal('session_warning_enabled', v)} />
                  <div className="flex justify-end">
                    <button type="button" onClick={saveLocalSettings} className="inline-flex items-center gap-2 min-h-touch px-4 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition">
              ...
              {activeTab === 'integrations' && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Track which integrations are enabled. Secret keys must stay in Base44 environment variables or backend functions.</p>
                  </div>
                  <IntegrationRow title="Gmail / SMTP" description="Used for outbound job documents, customer updates, and notifications." checked={localSettings.gmail_enabled} onChange={(v) => updateLocal('gmail_enabled', v)} />
                  <IntegrationRow title="Stripe" description="Used for subscriptions, invoice payments, and payment links." checked={localSettings.stripe_enabled} onChange={(v) => updateLocal('stripe_enabled', v)} />
                  <IntegrationRow title="Wave Accounting" description="Used for accounting sync and customer invoice workflows." checked={localSettings.wave_enabled} onChange={(v) => updateLocal('wave_enabled', v)} />
                  <IntegrationRow title="OpenAI" description="Used for photo analysis, restoration scope suggestions, and document assistance." checked={localSettings.openai_enabled} onChange={(v) => updateLocal('openai_enabled', v)} />
                  ...
                  <div className="flex justify-end">
                    <button type="button" onClick={saveLocalSettings} className="inline-flex items-center gap-2 min-h-touch px-4 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition">
                      <Save size={15} /> Save Integration Status
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="mt-6 bg-card rounded-xl border border-border divide-y divide-border">
            <div className="flex items-center justify-between px-5 py-4">
              ...
              <button type="button" onClick={handleReopenChecklist} className="min-h-touch px-3 rounded-lg border border-border text-xs text-primary font-medium hover:bg-muted transition">
                Reopen
              </button>
            </div>
          </div>
          ...
          <div className="mt-8 bg-red-50 border border-red-200 rounded-xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-800 mt-1">Permanently delete your account and associated account data. This action cannot be undone.</p>
                <button type="button" onClick={() => setShowDeleteModal(true)} className="min-h-touch mt-3 inline-flex items-center gap-2 px-4 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl max-w-md w-full p-6 relative">
            <button type="button" onClick={() => { setShowDeleteModal(false); setConfirmText(''); }} className="min-h-touch min-w-touch absolute top-4 right-4 text-muted-foreground hover:text-foreground transition">
              <X size={18} />
            </button>
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-2">This will permanently delete your account. This cannot be reversed.</p>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={confirmText} onChange={(e) => setConfirmText(e.target.value)} placeholder="Type DELETE" className={inputCls} />
              </div>
              <div className="flex gap-2">
                <button type="button" onClick={() => { setShowDeleteModal(false); setConfirmText(''); }} className="min-h-touch flex-1 rounded-lg border border-border text-sm font-medium hover:bg-muted transition">
                  Cancel
                </button>
                <button type="button" onClick={handleDeleteAccount} disabled={confirmText !== 'DELETE' || deleting} className="min-h-touch flex-1 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition disabled:opacity-60">
...
function ToggleRow({ title, description, checked, onChange, disabled }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-lg border border-border p-4">
      ...
      <button type="button" disabled={disabled} onClick={() => onChange(!checked)} className={`relative w-12 h-7 rounded-full transition disabled:opacity-60 ${checked ? 'bg-primary' : 'bg-muted'}`}>
...
function IntegrationRow({ title, description, checked, onChange }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-lg border border-border p-4">
      ...
      <button type="button" onClick={() => onChange(!checked)} className={`min-h-touch px-3 rounded-lg border text-xs font-semibold transition ${checked ? 'border-green-300 bg-green-50 text-green-700' : 'border-border text-muted-foreground hover:bg-muted'}`}>
