...
export default function NewJob() {
  ...
  const getCompanyId = () => {
    return userProfile?.company_id || user?.company_id || property?.company_id || insured?.company_id || '';
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!companyId) {
      setSubmitError('Missing company profile. Go to Settings and confirm your company profile is connected.');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.warn('[NewJob] Audit log failed:', auditError?.message || auditError);
      }

      navigate(`/jobs/${job.id}`);
    } catch (error) {
      console.error('[NewJob] Failed to create job:', error?.message || error);
      setSubmitError('Failed to create job. Check Job entity permissions and required fields.');
      setSaving(false);
    }
  };
          ...
          <div className="bg-card rounded-xl border border-border p-5 space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-sm font-medium text-right max-w-[60%]">
                {property
                  ? [property.address_line_1, property.city].filter(Boolean).join(', ')
                  : '—'}
              </span>
            </div>
          </div>
